error id: file:///C:/Users/Dayana/OneDrive/Escritorio/Algoritmos%20y%20Estructuras%20de%20Datos/Semana06_Practica/GuardadoH.java:java/io/PrintStream#println(+8).
file:///C:/Users/Dayana/OneDrive/Escritorio/Algoritmos%20y%20Estructuras%20de%20Datos/Semana06_Practica/GuardadoH.java
empty definition using pc, found symbol in pc: java/io/PrintStream#println(+8).
empty definition using semanticdb
empty definition using fallback
non-local guesses:

offset: 1142
uri: file:///C:/Users/Dayana/OneDrive/Escritorio/Algoritmos%20y%20Estructuras%20de%20Datos/Semana06_Practica/GuardadoH.java
text:
```scala
package Semana06_Practica;
import java.util.ArrayList;

public class GuardadoH {

    public static void main(String[] args) {
        ArrayList<String> Hentay=new ArrayList<>();

        Hentay.add("True B");
        Hentay.add("Shigokare Ecchi na Joshi Daisei to Doki x2 Love Lesson");
        Hentay.add("Ryoujoku Famiresu Choukyou Menu ");
        Hentay.add("Rennyuu Tales The Animation");
        Hentay.add("Real E S! TAni");
        Hentay.add("Pinkerton");
        Hentay.add("Niizuma Koyomi The Animation");
        Hentay.add("Nariyuki Papakatsu Girls!! The Animation");
        Hentay.add("Mou Hasamazu ni wa Irarenai Hoshi ni Onegai Shitara Konna ni Okkiku Nacchatta!");
        Hentay.add("Kimi no Mana wa Rina Witch ");
        Hentay.add("Juvenile Pornography The Animation");
        Hentay.add("Iizuka-senpai x Blazer Ane Kyun! yori The Animation");
        Hentay.add("Hatsujou Switch Otosareta Shoujo-tachi The Animation");
        Hentay.add("Hakoiri Shoujo - Virgin Territory");
        Hentay.add("Furueru Kuchibiru");
        Hentay.add("");
        Hentay.add("");

        System.out.printl@@n("Anime Hentay en listas para descarga.");
        int i=1;

        for (String descarga : Hentay){
            System.out.println( +i+ ". Hentay por descargar: " +descarga);
            i++;
        }



    }
    
}

```


#### Short summary: 

empty definition using pc, found symbol in pc: java/io/PrintStream#println(+8).